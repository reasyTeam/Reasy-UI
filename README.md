# reasy-ui-vue@v2
https://reasyteam.github.io/reasy-vue-cli/
...
| format          | 时间格式，hh:mm:ss，支持hh、mm、ss与任意字符组合，但不能为hms | string  | —        | hh:mm  |
| min             | 最小时间                                                     | string  | —        | —      |
| max             | 最大时间                                                     | string  | —        | —      |
...
| empty    | 空数据时显示的文本内容，可使用slot="empty"设置 | string/slot           | -      | 暂无数据 |
...
| 参数        | 说明                                   | 类型               | 可选值 | 默认值 |
| ----------- | -------------------------------------- | ------------------ | ------ | ------ |
| avatar      | 图标资源路径配置，不配置则不显示       | string/object/slot | -      | -      |
| title       | 列表子项的标题配置，不配置则不显示     | string/slot        | -      | -      |
| desc | 列表子项的描述文字配置，不配置则不显示 | string/slot        | -      | -      |

注：类型列，string：表示该字段的值，object如下：
...
| enterable     | 鼠标是否可以进入popover中，当trigger为“hover”时 | boolean     | -                                                            | true          |
| open-delay    | 延迟出现，单位：毫秒      | number      | -                                                            | 0             |
| close-delay   | 延迟出现，单位：毫秒      | number      | -                                                            | 0             |
...
| display-options-number  | 设置可显示选项的数量，如options.length > 此值，显示滚动条   | number  | - | 4    |
...
| 参数          | 说明             | 类型    | 可选值           | 默认值             |
| ------------- | ---------------- | ------- | ---------------- | ------------------ |
| value/v-model | 当前激活tab的key | string  | -                | 第一个选项卡的name |
| type          | 风格类型         | string  | link/card/button | link               |
| size          | 大小             | string  | S/M/L            | M                  |
| lazy          | true使用v-if加载，false使用v-show | string  | -               | true               |
...
| 事件名称    | 说明                                                         | 参数                      |
| ----------- | ------------------------------------------------------------ | ------------------------- |
| before-leave | 切换标签之前的钩子，若返回false或者返回Promise且被reject，则阻止切换 | activeName, oldActiveName |
...
| space          | 每个step的间距，单位：px。不填写将自适应间距 | number  | -                                  | -       |
...
| icon        | 传入icon的class全名来自定义icon               | string/slot | -                                  | -      |
...
| 参数                 | 说明                               | 类型    | 可选值 | 默认值 |
| -------------------- | ---------------------------------- | ------- | ------ | ------ |
| visible/v-model      | 是否显示dialog                     | boolean | -      | false  |
| title                | dialog的标题，也可通过具名slot传入 | -       | -      | -      |
| width                | dialog的宽度                       | string  | -      | 400px  |
...
| confirm-button-text | 确认按钮文字                                                 | string        | -                                                            | 确定      |
| cancel-button-text  | 取消按钮文字                                                 | string        | -                                                            | 取消      |
| confirm-button-type | 确认按钮类型                                                 | string        | 参考button组件类型                                           | [todo]    |
| cancel-button-type  | 取消按钮类型                                                 | string        | 参考button组件类型                                           | [todo]    |
...
| width      | 宽，单位px        | number  | -               | 外层容器的宽 |
| height     | 高，单位px        | number  | -               | 外层容器的高 |
| stroke-width | 线条的粗细，单位px | number  | -  | 10 |
