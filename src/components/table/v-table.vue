  ...
  <div class="form-group" v-show="tableOptions.show" :class="tableOptions.css">
    <div class="table-search" v-if="tableOptions.search">
      ...
      <div
        ...
        v-if="!supportPlaceholder && !searchValue"
      >
        {{ searchText }}
      </div>
    ...
    >
      <thead>
        ...
        <tr>
          <th width="64px" v-if="tableOptions.selectBox">
          ...
          <th
            :width="columns.width"
            v-for="columns in visibleColumns"
            :key="columns.field"
          >
            <span
            ...
            >
              {{ columns.title }}
              <svg v-if="columns.sort" class="sort-column">
                <path
                  d="M 4 1 L 7.5 5 L 0.5 5"
                  :class="{
                    'sort-active':
                      sortType === 'asc' && sortKey == columns.field,
                  }"
                ></path>
                <path
                  d="M 4 11 L 7.5 7 L 0.5 7"
                  :class="{
                    'sort-active':
                      sortType === 'desc' && sortKey == columns.field,
                  }"
                ></path>
              </svg>
              <!-- <span
            ...
            <template v-for="columns in tableOptions.columns">
              ...
              >
                ...
                <span
                  ...
                  v-tooltip="columns.showTooltip ? rowsData[columns.field] : ''"
                  style="cursor: text;"
                  :class="columns.css"
            ...
            <td
              :colspan="
                tableOptions.selectBox
                  ? tableOptions.columns.length + 1
                  : tableOptions.columns.length
              "
            >
              <div class="table-no-data v-icon-no-data">{{ noData }}</div>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="table-footer clearfix" v-if="showNormalFooter">
      ...
      <div class="footer-page">
        <a
          class="table-btn v-icon-arrrow-up page-change-arrow"
          @click="gotoPage('prev')"
          :class="{ disabled: tableOptions.page === 0 }"
        ></a>
        <a
          class="table-btn"
        ...
        >
        <a
          class="table-btn v-icon-arrrow-down page-change-arrow"
          @click="gotoPage('next')"
          :class="{ disabled: tableOptions.page >= tableOptions.totalPage - 1 }"
        ></a>
      </div>
    </div>
    <div v-else class="simple-table-footer">
...
import { copyDeepData, sortByKey, sortFnNames } from "../libs";
...
export default {
  ...
  created() {
    ...
    let pagePerNum = this.tableOptions.pagePer,
  ...
  methods: {
    ...
    sortTable(fieldOptions, field) {
      ...
      // add by xc 校正sort的值
      let sortFn = fieldOptions.sort;
      if (typeof sortFn !== "function") {
        if (sortFnNames.names.indexOf(sortFn) < 0) {
          sortFn = sortFnNames.default;
        }
      }
      //排序元素
      this.sortKey = field;
    ...
    sortData(data, sortConfig) {
      let _this = this;
      data = data || [];
      return _this.sortKeyOri
        ? data.sort(function (a, b) {
            return sortByKey(a, b, _this.sortKeyOri, sortConfig);
          })
        : data;
    },
